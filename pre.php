...
if ($_POST['action'] == 0  and $_SERVER['REMOTE_ADDR'] == "83.221.178.214") {
  ...
  $action = $_POST['action'];
  $error = $_POST['error'];
  $md5 = md5($click_trans . $service_id . $key . $merchant_trans_id . $amount . $action . $sign_time);
  $db->insert("click_uz", ['click_trans_id' => $click_trans, 'merchant_trans_id' => $merchant_trans_id, 'amount' => $amount, 'amount_rub' => $amount, 'sign_time' => $sign_time, 'situation' => $error]);
  ...
  if (strlen($row['id']) > 0) {
    ...
    $array = array(
      ...
      'merchant_prepare_id' => $prepare,
      'error' => -9,
      'error_note' => "Do not find a user!!! created by dadabayev.uz"
    );
    echo json_encode($array);
  }
  }else{
